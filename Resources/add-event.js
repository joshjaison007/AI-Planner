...
modalDiv.innerHTML = `
    <div style="background: #1a1b2e; padding: 35px; border-radius: 30px; width: 400px; border: 1px solid rgba(255,255,255,0.1);">
        ...
        <input type="time" id="evTime" style="width:100%; padding:12px; border-radius:20px; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); color:white; margin-bottom:15px; color-scheme:dark;">
        <div style="margin-bottom: 25px;">
            <label style="color: var(--text-dim); font-size: 12px; margin-left: 10px;">Duration (minutes)</label>
            <input type="number" id="evDuration" value="30" style="width:100%; padding:12px; border-radius:20px; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); color:white; color-scheme:dark;">
        </div>
...
window.saveEvent = async () => {
    ...
    const duration = document.getElementById('evDuration').value || 30;
    ...
    try {
        ...
        if (res.ok) {
            ...
            togglePopup(false);
            if (typeof window.syncWithMCP === 'function') {
                window.syncWithMCP();
            }
        } else {
